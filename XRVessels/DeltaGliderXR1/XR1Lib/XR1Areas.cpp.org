...
//
// Copyright 2006-2007 Douglas E. Beachy
// All rights reserved.
//
...
XR1Area::XR1Area(InstrumentPanel &parentPanel, const COORD2 panelCoordinates, const int areaID) :
    Area(parentPanel, panelCoordinates, areaID)
{
    // TODO: refactor this when the XR2's active VC is created
    // NOTE: with the advent of vessel subclasses, vcmesh_tpl may be null
    if (GetXR1().vcmesh_tpl != NULL)
    {
        // retrieve VC texture handles from our vessel; we do not need to clean these up later
        m_tex1 = oapiGetTextureHandle(GetXR1().vcmesh_tpl, 16);
        m_tex2 = oapiGetTextureHandle(GetXR1().vcmesh_tpl, 18);
        m_tex3 = oapiGetTextureHandle(GetXR1().vcmesh_tpl, 14);
    }
}
...
VerticalCenteringRockerSwitchArea::VerticalCenteringRockerSwitchArea(InstrumentPanel &parentPanel, const COORD2 panelCoordinates, const int areaID, bool isDual, bool reverseRotation, POSITION initialPosition) :
    XR1Area(parentPanel, panelCoordinates, areaID),
    m_isDual(isDual), m_pAnimationHandle(NULL), m_reverseRotation(reverseRotation), m_initialPosition(initialPosition)
{
}
...
bool VerticalCenteringRockerSwitchArea::Redraw(const int event, const SURFHANDLE surf)
{
    if (isVC())  // 3D
    {
...
// isDual: true = is dual switches, false = single switch
// initialPosition: defaults to CENTER if not set; if not CENTER, switch will not auto-center
HorizontalCenteringRockerSwitchArea::HorizontalCenteringRockerSwitchArea(InstrumentPanel &parentPanel, const COORD2 panelCoordinates, const int areaID, bool isDual, bool reverseRotation, POSITION initialPosition) :
    XR1Area(parentPanel, panelCoordinates, areaID),
    m_isDual(isDual), m_pAnimationHandle(NULL), m_reverseRotation(reverseRotation), m_initialPosition(initialPosition)
{
}
...
bool HorizontalCenteringRockerSwitchArea::Redraw(const int event, const SURFHANDLE surf)
{
    if (isVC())  // 3D
    {
        if (m_pAnimationHandle) // any 3D switch to animate?
...
IndicatorGaugeArea::IndicatorGaugeArea(InstrumentPanel &parentPanel, const COORD2 panelCoordinates, const int areaID, const bool isDual, 
                                       const int redrawFlag, const int deltaX, const int deltaY, const int gapSize) :
    XR1Area(parentPanel, panelCoordinates, areaID), 
    m_isDual(isDual), m_redIndicatorSurface(NULL), m_yellowIndicatorSurface(NULL), m_redrawFlag(redrawFlag), 
...
void IndicatorGaugeArea::Activate()
{
    ...
    oapiSetSurfaceColourKey(m_mainSurface, white);
    oapiSetSurfaceColourKey(m_redIndicatorSurface, white);
    oapiSetSurfaceColourKey(m_yellowIndicatorSurface, white);
    ...
    if (isVC()) // 3D panel?
    {
        oapiVCRegisterArea(GetAreaID(), GetRectForSize(sizeX + m_deltaX, sizeY + m_deltaY), m_redrawFlag, PANEL_MOUSE_IGNORE, PANEL_MAP_BGONREQUEST, GetTex1());
    }
    else    // 2D panel
    {
...
VerticalGaugeArea::VerticalGaugeArea(InstrumentPanel &parentPanel, const COORD2 panelCoordinates, const int areaID, const bool isDual, const int sizeY, 
                                     const int redrawFlag, const int deltaX, const int deltaY, const int gapSize, const SIDE singleSide) :
    IndicatorGaugeArea(parentPanel, panelCoordinates, areaID, isDual, redrawFlag, deltaX, deltaY, gapSize),
...
bool VerticalGaugeArea::Redraw(const int event, const SURFHANDLE surf)
{
...
bool HorizontalGaugeArea::Redraw(const int event, const SURFHANDLE surf)
{
    bool retVal = false;    // assume not re-rendered
...
bool SimpleButtonArea::Redraw(const int event, const SURFHANDLE surf)
{
    if (isVC())  // 3D
    {
        static const float tv0[8] = {0,0,0.0469f,0.0469f,0,0,0.0469f,0.0469f};

        if ((GetXR1().vcmesh == NULL) || (m_buttonMeshGroup < 0))
...
void TimedButtonArea::clbkPostStep(const double simt, const double simdt, const double mjd)
{
...
// analog gauge area
AnalogGaugeArea::AnalogGaugeArea(InstrumentPanel &parentPanel, const COORD2 panelCoordinates, const int areaID, const double initialAngle) :
    XR1Area(parentPanel, panelCoordinates, areaID),
...
void AnalogGaugeArea::Activate()
{
    if (isVC())  // 3D panel
    {
        oapiVCRegisterArea(GetAreaID(), GetRectForSize(56, 56), PANEL_REDRAW_ALWAYS, PANEL_MOUSE_IGNORE, PANEL_MAP_BGONREQUEST, GetTex2());
    }
    else    // 2D
    {
...
bool AnalogGaugeArea::Redraw(const int event, const SURFHANDLE surf)
{
    bool retVal = false;    // assume not re-rendered
...
void AnalogGaugeArea::DrawNeedle(HDC hDC, int x, int y, double rad, double angle, double speed)
{
    ...
    MoveToEx(hDC, x, y, 0); 
    LineTo(hDC, x + (int)(0.85*dx+0.5), y - (int)(0.85*dy+0.5));
    
    SelectObject(hDC, m_pen0);
    MoveToEx(hDC, x, y, 0); 
    LineTo(hDC, x + (int)(dx+0.5), y - (int)(dy+0.5));
...
bool ToggleSwitchArea::Redraw(const int event, const SURFHANDLE surf)
{
...
bool LEDArea::Redraw(const int event, const SURFHANDLE surf)
{
    if (m_isOn)
    {
...
DoorIndicatorArea::DoorIndicatorArea(InstrumentPanel &parentPanel, const COORD2 panelCoordinates, const int areaID, const DoorStatus *pDoorStatus, const int surfaceIDB, const double *pAnimationState) :
    XR1Area(parentPanel, panelCoordinates, areaID),
...
void DoorIndicatorArea::Activate()
{
    if (isVC())
    {
        oapiVCRegisterArea (GetAreaID(), GetRectForSize(43, 31), PANEL_REDRAW_USER, PANEL_MOUSE_IGNORE, PANEL_MAP_BACKGROUND, GetTex1());
    }
    else  // 2D
    {
...
bool DoorIndicatorArea::Redraw(const int event, const SURFHANDLE surf)
{
...
// Blink "Transit"
void DoorIndicatorArea::clbkPostStep(const double simt, const double simdt, const double mjd)
{
    // compute door transit index if we have animation in progress
    if (m_pAnimationState != NULL)
    {
        int newTransitIndex = 2 + (int)(27 * *m_pAnimationState);    // pixel index
...
bool BarArea::Redraw(const int event, const SURFHANDLE surf)
{
    bool retVal = false;
    ...
    if (renderData != m_lastRenderData)
    {
        ...
        if (brightIndex > 0)  // anything on the gauge at all?
        {
...
bool NumberArea::Redraw(const int event, const SURFHANDLE surf)
{
...
PctHorizontalGaugeArea::PctHorizontalGaugeArea(InstrumentPanel &parentPanel, const COORD2 panelCoordinates, const int areaID, const bool isDual, const int sizeX, 
                                         const int redrawFlag, const int deltaX, const int deltaY, const int gapSize) :
    HorizontalGaugeArea(parentPanel, panelCoordinates, areaID, isDual, sizeX, redrawFlag, deltaX, deltaY, gapSize)
{
}
...
HorizontalGaugeArea::RENDERDATA PctHorizontalGaugeArea::GetRenderData(const SIDE side)
{
    ...
    int index = (int)((maxIndex * fraction) + 0.5);  // round to nearest pixel
...
bool AccNumberArea::UpdateRenderData(RENDERDATA &renderData)
{
    ...
    acc = ((double)((int)((acc + 0.0005) * 1000))) / 1000;
...
AccHorizontalGaugeArea::AccHorizontalGaugeArea(InstrumentPanel &parentPanel, const COORD2 panelCoordinates, const int areaID, const AXIS axis, const bool isDual, const SIDE side) :
    HorizontalGaugeArea(parentPanel, panelCoordinates, areaID, isDual, 91, PANEL_REDRAW_ALWAYS, 0, 0, -1, side),  // gap of 0 for dual indicator
    m_axis(axis)
{
}
...
HorizontalGaugeArea::RENDERDATA AccHorizontalGaugeArea::GetRenderData(const SIDE side)
{
    ...
    fraction = (double)((int)((fraction + .0005) * 1000)) / 1000;
    ...
    // compute pixel
    const int maxIndex = 84;    // total width = 85 pixels (index 0-84, inclusive)
    int index = (int)((maxIndex * absFraction) + 0.5);  // round to nearest pixel
...
bool AccScaleArea::Redraw(const int event, const SURFHANDLE surf)
{
    bool redraw = false;
...
COLORREF XR1Area::GetTempCREF(double tempK, double limitK, int doorStatus)
{
...
COLORREF XR1Area::GetValueCREF(double value, double warningLimit, double criticalLimit)
{
    COLORREF retVal;
...
bool MomentaryButtonArea::Redraw(const int event, const SURFHANDLE surf)
{
    if (isVC())  // 3D
    {
        static const float tv0[8] = {0,0,0.0469f,0.0469f,0,0,0.0469f,0.0469f};
...
bool MomentaryButtonArea::ProcessMouseEvent(const int event, const int mx, const int my)
{
    ...
    if (event & PANEL_MOUSE_LBDOWN)
    {
        m_buttonDownSimt = oapiGetSimTime();    
        m_isLit = true;
    }
...
bool RawButtonArea::Redraw(const int event, const SURFHANDLE surf)
{
    if (isVC())  // 3D
    {
        static const float tv0[8] = {0,0,0.0469f,0.0469f,0,0,0.0469f,0.0469f};
...
bool TimerNumberArea::RenderTimeValue(RENDERDATA &renderData, double time)
{
    ...
    else    // hours, minutes, or seconds
    {
        ...
        value = (int)elapsedUnitsInCurrentDay % (m_maxValue + 1);   // 0...m_maxValue
    }
...
LargeBarArea::LargeBarArea(InstrumentPanel &parentPanel, const COORD2 panelCoordinates, const int areaID, int sizeX, int sizeY, int resourceID, int darkResourceID) :
    ...
    m_resourceID(resourceID), m_darkResourceID(darkResourceID), m_darkSurface(NULL)
{
}
...
bool LargeBarArea::Redraw(const int event, const SURFHANDLE surf)
{
    bool retVal = false;
    ...
    if (renderData != m_lastRenderData)
    {
        ...
        if (m_darkSurface != NULL)  // any dark surface defined?
        {
            //            texture        tgtX  tgtY       srcX  srcY       width    height
...
LargeFuelBarArea::LargeFuelBarArea(InstrumentPanel &parentPanel, const COORD2 panelCoordinates, const int areaID, PROPELLANT_HANDLE ph, const int resourceID, const int darkResourceID, const double gaugeMinValue) :
    ...
    m_maxFuelQty(-1), m_pFuelRemaining(NULL), m_propHandle(ph), m_gaugeMinValue(gaugeMinValue)
{
}
...
LargeFuelBarArea::LargeFuelBarArea(InstrumentPanel &parentPanel, const COORD2 panelCoordinates, const int areaID, double maxFuelQty, const double *pFuelRemaining, const int resourceID, const int darkResourceID, const double gaugeMinValue) :
    ...
    m_maxFuelQty(maxFuelQty), m_pFuelRemaining(pFuelRemaining), m_propHandle(NULL), m_gaugeMinValue(gaugeMinValue)
{
}
...
bool FuelDumpButtonArea::Redraw(const int event, const SURFHANDLE surf)
{
    // always re-render this since it is always performed on request
...
void FuelDumpButtonArea::ProcessButtonPressed(const int event)
{
    ...
    if (event & PANEL_MOUSE_LBDOWN)
    {
        ...
        {
            char temp[45];
            m_fuelDumpInProgress = false;       // fuel dump halted now
            GetXR1().PlaySound(GetXR1().BeepLow, DeltaGliderXR1::ST_Other);
            sprintf(temp, "%s fuel dump halted.", m_fuelLabel);
            GetXR1().ShowInfo(NULL, DeltaGliderXR1::ST_None, temp);
    ...
    const double buttonHoldTime = oapiGetSimTime() - m_buttonDownSimt;
    ...
    if (event & PANEL_MOUSE_LBPRESSED)
    {
        ...
        {
            // start the fuel dump
            m_fuelDumpInProgress = true;
            GetXR1().PlaySound(GetXR1().BeepHigh, DeltaGliderXR1::ST_Other);

            char temp[45];
            // we need a message here so it displays instantly
            sprintf(temp, "WARNING: %s fuel dump in progress.", m_fuelLabel);  // NOTE: must match the msg in the PostStep so we don't print two messages
...
void FuelDumpButtonArea::clbkPostStep(const double simt, const double simdt, const double mjd)
{
    if (m_fuelDumpInProgress)
    {
...
bool SupplyHatchToggleSwitchArea::ProcessSwitchEvent(bool switchIsOn)
{
    ...
    const bool doorUnlocked = ((isGrounded && (airspeed == 0)) || isDocked);
    ...
    if (GetXR1().m_pConfig->EnableResupplyHatchAnimationsWhileDocked)
...
bool DoorMediumLEDArea::Redraw(const int event, const SURFHANDLE surf)
{
