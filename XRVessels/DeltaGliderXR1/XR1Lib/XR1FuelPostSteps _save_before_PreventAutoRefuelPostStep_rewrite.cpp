...
//
// Copyright 2006-2010 Douglas E. Beachy
// All rights reserved.
//
...
APUPostStep::APUPostStep(DeltaGliderXR1 &vessel) : 
    XR1PrePostStep(vessel),
    m_prevDoorStatus(NOT_SET), m_doorTargetSimt(0), m_prevQty(-1), m_firstTimeStep(true)
{
}
...
void APUPostStep::clbkPrePostStep(const double simt, const double simdt, const double mjd)
{
    ...
    if (AFCtrlOn)
    {
        GetXR1().m_latestHydraulicDoorRunningSimt = simt;   // reset callout time so that countdown starts when AF CTRL turned OFF
    }
    else  // AF CTRL = OFF
    {
        ...
        if (pConfig->APUIdleRuntimeCallouts > 0)        // callouts enabled?
        {
            ...
            {
                double deltaFromLastLoad = simt - GetXR1().m_latestHydraulicDoorRunningSimt;
                if (deltaFromLastLoad >= pConfig->APUIdleRuntimeCallouts)
                {
                    ...
                    // reset the callout time
                    GetXR1().m_latestHydraulicDoorRunningSimt = simt;
                }
            }
        }
    }
...
void APUPostStep::UpdateAPUDoorState(const double simt, const double simdt, const double mjd)
{
    ...
    if (m_firstTimeStep)
    {
        ...
        return;     // wait until Orbiter and OrbiterSound finishes initializing
    }

    DoorStatus doorStatus = GetXR1().apu_status;

    // check whether we just reached m_doorTargetSimt
    if ((m_doorTargetSimt > 0) && (simt >= m_doorTargetSimt))
    {
        ...
        m_doorTargetSimt = 0;  // reset for next time
    ...
    if ((doorStatus != DOOR_FAILED) && (doorStatus != m_prevDoorStatus))
    {
        ...
        {
            GetXR1().LoadXR1Sound(GetXR1().APU, "APU Startup.wav", INTERNAL_ONLY);
        ...
        {
            GetXR1().LoadXR1Sound(GetXR1().APU, "APU Shutdown.wav", INTERNAL_ONLY);
            GetXR1().PlaySound(GetXR1().APU, DeltaGliderXR1::ST_Other, APU_VOL);
            m_doorTargetSimt = simt + spinupSpindownTime;
        ...
        {
            GetXR1().LoadXR1Sound(GetXR1().APU, "APU Run.wav", INTERNAL_ONLY);
...
FuelDumpPostStep::FuelDumpPostStep(DeltaGliderXR1 &vessel) : 
    XR1PrePostStep(vessel),
    m_nextWarningSimt(-1), m_fuelDumpStream1(NULL), m_fuelDumpStream2(NULL), 
    m_fuelDumpLevel(0)
{
    // create the particle streams if the parent vessel supports them
    if (GetXR1().m_pFuelDumpParticleStreamSpec != NULL)
...
{
    if (m_fuelDumpStream1 != NULL)
        GetVessel().DelExhaustStream(m_fuelDumpStream1);

    if (m_fuelDumpStream2 != NULL)
...
void FuelDumpPostStep::clbkPrePostStep(const double simt, const double simdt, const double mjd)
{
    ...
    double dumpFlowRate = 0.0;      // 0 -> 1.0; used for dump particle level
    // flow weights:
    //   Main:  50%
    //   SCRAM: 25%
    //   LOX: 15%
    ...
    {
        DumpFuel(GetXR1().ph_main, simdt, GetXR1().m_mainFuelDumpInProgress, 1.0);
        fuelFlowing = true;
        dumpFlowRate += 0.50;
    }
    ...
    {
        DumpFuel(GetXR1().ph_rcs, simdt, GetXR1().m_rcsFuelDumpInProgress, RCS_FLOW_FRACTION);
        fuelFlowing = true;
        dumpFlowRate += 0.05;
    }
    ...
    {
        DumpFuel(GetXR1().ph_scram, simdt, GetXR1().m_scramFuelDumpInProgress, SCRAM_FLOW_FRACTION);
        fuelFlowing = true;
        dumpFlowRate += 0.25;
    }
    ...
    if (GetXR1().m_apuFuelDumpInProgress)
    {
        ...
        if (GetXR1().m_apuFuelQty > 0)
        {
            ...
            if (GetXR1().m_apuFuelQty <= 0)    // just reached empty?
            {
                GetXR1().m_apuFuelDumpInProgress = false;  
            }
            else
            {
                fuelFlowing = true;
    ...
    if (GetXR1().m_loxDumpInProgress)
    {
        ...
        if (GetXR1().GetXRLOXMass() > 0)
        {
            ...
            if (qty <= 0)    // just reached empty?
            {
                GetXR1().m_loxDumpInProgress = false;  
    ...
    // TESTING ONLY: dumpFlowRate = 1.0;
    m_fuelDumpLevel = dumpFlowRate;   // 0...1.0
    // DEBUG: sprintf(oapiDebugString(), "dumpFlowRate=%lf", dumpFlowRate);  

    // manage the fuel flow sound
    if (fuelFlowing)
    {
        // handle fuel/lox flow sounds
        int flowCount = static_cast<int>(GetXR1().m_mainFuelDumpInProgress) + 
        ...
        else  // all flow is HALTED
        {
            ...
            m_fuelDumpLevel = 0;    // halt
        }

        // show a warning every 5 seconds while any fuel dump is in progress 
        if (simt >= m_nextWarningSimt)
        {
            char temp[45];
    ...
    else    // fuel not flowing
    {
        ...
        m_nextWarningSimt = -1;   // reset
    }
}

// rateFraction = fraction of speed to dump this tank
void FuelDumpPostStep::DumpFuel(const PROPELLANT_HANDLE ph, const double simdt, bool &dumpInProgress, const double rateFraction)
{
    ...
    if (remaining > 0)
    {
        ...
        if (remaining <= 0)    // just reached empty?
        {
            remaining = 0;
...
void XFeedPostStep::clbkPrePostStep(const double simt, const double simdt, const double mjd)
{
    ...
    if (mainToRCSFlow != 0)
    {
        ...
        if (haltFlow)
        {
            GetXR1().m_xfeedMode = XF_OFF;
            char temp[80];
            sprintf(temp, "%s; cross-feed OFF.", pMsg);
            GetXR1().ShowInfo("Cross-Feed Off.wav", DeltaGliderXR1::ST_InformationCallout, temp);

            // refresh the xfeed knob
            GetXR1().TriggerRedrawArea(AID_XFEED_KNOB);
...
{
    const double isGrounded = GetVessel().GroundContact();
    const double isDocked = GetXR1().IsDocked();
    ...
    bool resupplyEnabled = ((isGrounded && (airspeed == 0)) || isDocked);
    ...
    /* NOTE: we need to work around some odd Orbiter core issue here: 
       ...
       frame until ground contact is reestablished the next frame:
        ...
        As a workaround, we only break resupply contact if we've been moving for more than one 2/10th of a second.
    */
    if (resupplyEnabled == false) 
    {
        ...
        const double movementTime = simt - m_resupplyMovementFirstDetectedSimt;
    ...
    if (resupplyEnabled)
    {
        ...
        if (GetXR1().fuelhatch_status == DOOR_OPEN)
        {
            ...
            if (m_prevFuelHatchStatus != DOOR_OPEN)
            {
                ...
                GetXR1().LoadXR1Sound(GetXR1().FuelResupplyLine, "Resupply Line Extend.wav", INTERNAL_ONLY);
            ...
            if (simt >= m_refuelingSequenceStartSimt)
            {
                ...
                {
                    GetXR1().LoadXR1Sound(GetXR1().FuelResupplyLine, "Resupply Line Attach.wav", INTERNAL_ONLY);
                    ...
                    const bool isGrounded = GetVessel().GroundContact();
        ...
        if (GetXR1().loxhatch_status == DOOR_OPEN)
        {
            ...
            if (m_prevLoxHatchStatus != DOOR_OPEN)
            {
                ...
                GetXR1().LoadXR1Sound(GetXR1().LoxResupplyLine, "Resupply Line Extend.wav", INTERNAL_ONLY);
            ...
            if (simt >= m_loxSequenceStartSimt)
            {
                ...
                {
                    GetXR1().LoadXR1Sound(GetXR1().LoxResupplyLine, "Resupply Line Attach.wav", INTERNAL_ONLY);
        ...
        if (GetXR1().externalcooling_status == DOOR_OPEN)
        {
            ...
            if (m_prevExternalCoolingStatus != DOOR_OPEN)
            {
                ...
                GetXR1().LoadXR1Sound(GetXR1().ExternalCoolingLine, "Resupply Line Extend.wav", INTERNAL_ONLY);
            ...
            if (simt >= m_externalCoolingSequenceStartSimt)
            {
                ...
                {
                    GetXR1().LoadXR1Sound(GetXR1().ExternalCoolingLine, "Resupply Line Attach.wav", INTERNAL_ONLY);
...
void ResupplyPostStep::PerformRefueling(const double simt, const double simdt, const double mjd)
{
    ...
    if (mainFuelAvailable)
    {
        if (GetXR1().m_mainFuelFlowSwitch)
            FlowMainFuel(simt, simdt, mjd);
    }
    
    if (scramFuelAvailable)
    {
        if (GetXR1().m_scramFuelFlowSwitch)
            FlowScramFuel(simt, simdt, mjd);
    }

    if (apuFuelAvailable)
    {
        if (GetXR1().m_apuFuelFlowSwitch)
...
void ResupplyPostStep::PerformLoxResupply(const double simt, const double simdt, const double mjd)
{
    ...
    if (isDocked)
        ...
        loxAvailable  = (config.AllowEarthOnlyResupply[TANK_LOX] ? onEarth : config.AllowGroundResupply[TANK_LOX]);
...
void LOXConsumptionPostStep::clbkPrePostStep(const double simt, const double simdt, const double mjd)
{
    ...
    if (m_previousO2Level > 0)
    {
        ...
        if (ambientO2Available)
        {
            ...
            GetXR1().XRSoundOnOff(PLAYCABINAIRCONDITIONING, true);
        }
        else  // no ambient O2 available (using internal O2)
        {
            ...
            GetXR1().XRSoundOnOff(PLAYCABINAIRCONDITIONING, loxAvailable);  // no internal airflow if lox not available
        }
        ...
        if ((GetXR1().m_crewState != DEAD) && (GetXR1().GetCrewMembersCount() > 0))
        {
            ...
            else if ((o2Level <= CRITICAL_O2_LEVEL_WARNING) && (m_previousO2Level > CRITICAL_O2_LEVEL_WARNING))  // only play this once
            {
                ...
                GetXR1().XRSoundOnOff(PLAYRADIOATC, false);  
            }
            else if ((o2Level > (CRITICAL_O2_LEVEL_WARNING + 0.01)) && (m_previousO2Level <= (CRITICAL_O2_LEVEL_WARNING + 0.01)))
            {
                ...
                // re-enable ATC 
                GetXR1().XRSoundOnOff(PLAYRADIOATC, true);  
            }
        }
    }
...
// NOTE: be careful to only check the ship's *internal* main fuel tank here, *not* the bay tanks (if any).
    ...
    XR1PrePostStep(vessel)
{
    ...
    m_previousFuelQty[0] = GetXR1().m_startupMainFuelFrac * GetXR1().GetXRPropellantMaxMass(GetXR1().ph_main);
    m_previousFuelQty[1] = GetXR1().m_startupRCSFuelFrac * GetXR1().GetXRPropellantMaxMass(GetXR1().ph_rcs);
    m_previousFuelQty[2] = GetXR1().m_startupSCRAMFuelFrac * GetXR1().GetXRPropellantMaxMass(GetXR1().ph_scram);

    // NOTE: must delay the real initialization of m_previousFuelMaxQty until our clbkPrePostStep because the child vessels are not
    // marked as attached in the bay yet, causing GetXRPropellantMaxMass to always return zero even if child fuel tanks
    // are attached in the bay: the constructor is invoked from clbkPostCreation.
    // For now, set each to -1, which means "not set yet"
    for (int i=0; i < 3; i++)
        m_previousFuelMaxQty[i] = -1;
}

void PreventAutoRefuelPostStep::clbkPrePostStep(const double simt, const double simdt, const double mjd)
{
    // m_previousFuelMaxQty must be one-time initialized here (see note in constructor)
    if (m_previousFuelMaxQty[0] < 0)  // just checking the first entry is sufficient since all three are in lock-step
    {
        m_previousFuelMaxQty[0] = GetXR1().GetXRPropellantMaxMass(GetXR1().ph_main);
        m_previousFuelMaxQty[1] = GetXR1().GetXRPropellantMaxMass(GetXR1().ph_rcs);
        m_previousFuelMaxQty[2] = GetXR1().GetXRPropellantMaxMass(GetXR1().ph_scram);
    }
    ...
    // only disable refueling if we are not actively refueling or cross-feeding
    if ((GetXR1().m_mainFuelFlowSwitch == false) && 
        (GetXR1().m_scramFuelFlowSwitch == false) &&
        (GetXR1().m_xfeedMode != XF_MAIN) &&
        (GetXR1().m_xfeedMode != XF_RCS))
    {
        DisableAutoRefuel(GetXR1().ph_main,  0);
        DisableAutoRefuel(GetXR1().ph_rcs,   1);
        DisableAutoRefuel(GetXR1().ph_scram, 2);
    }
...
// index = index into m_previousFuelQty array
void PreventAutoRefuelPostStep::DisableAutoRefuel(PROPELLANT_HANDLE ph, const int index)
{
    double fuelQty = GetXR1().GetXRPropellantMass(ph);     // can't be const because we may set it below
    const double maxFuelQty = GetXR1().GetXRPropellantMaxMass(ph);
    const double prevFuelQty = m_previousFuelQty[index];

    if (prevFuelQty >= 0)  // only check if we have valid data
    {
        // check whether fuel qty went UP since last timestep
        if (fuelQty > prevFuelQty)
        {
            // We want to ALLOW payload tanks.  Therefore, check whether the *max fuel quantity* 
            // has changed as well.  If not, then Orbiter is refueling us.  If it *did* change,
            // however, then it means that a payload tank was added, so we want to allow that.
            if (maxFuelQty == prevFuelQty)
            {
                // Orbiter is refueling us!  Back out the fuel change.
                // NOTE: this should never affect the bay tanks!
                GetXR1().SetXRPropellantMass(ph, prevFuelQty);
                fuelQty = prevFuelQty;      // preserve existing fuel qty
            }
        }
    }

    // remember these values for the next frame
    m_previousFuelQty[index] = fuelQty;     
    m_previousFuelMaxQty[index] = maxFuelQty;
}
...
BoilOffPostStep::BoilOffPostStep(DeltaGliderXR1 &vessel) : 
    XR1PrePostStep(vessel),
    m_stream1(NULL), m_stream2(NULL), m_level(0)
{
    // create the particle streams if the parent vessel supports them
    if (GetXR1().m_pBoilOffExhaustParticleStreamSpec != NULL)
