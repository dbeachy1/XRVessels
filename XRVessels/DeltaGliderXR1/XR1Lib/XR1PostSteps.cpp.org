...
AttitudeHoldPreStep::AttitudeHoldPreStep(DeltaGliderXR1 &vessel) : 
    ...
    m_forceOnlineCallout(false), m_lastRCSMaxThrustUpdateSimt(-1)
{
    ResetLearningData();
...
void AttitudeHoldPreStep::clbkPrePostStep(const double simt, const double simdt, const double mjd)
{
    ...
    if ((customAutopilotMode == AP_ATTITUDEHOLD) || (descentHoldActive))
    {
        ...
        // update RCS max thrust levels once per second to handle any payload mass changes; this only applies to subclasses
        if ((simt - m_lastRCSMaxThrustUpdateSimt) > 1.0)
        {
            GetXR1().ResetAllRCSThrustMaxLevels();
            m_lastRCSMaxThrustUpdateSimt = simt;
        }
    }
    ...
    if (((customAutopilotMode == AP_ATTITUDEHOLD) || descentHoldActive) && (m_prevCustomAutopilotMode != AP_NOTSET))
    {
        ...
        if ((GetXR1().InAtm() == false) && (GetXR1().m_centerOfLift != NEUTRAL_CENTER_OF_LIFT))
        {
            ...
            GetXR1().m_cogShiftCenterModeActive = true;
            GetXR1().TriggerRedrawArea(AID_COG_CENTER_BUTTON);
        }
        ...
        // handle BANK
        const double targetBank = (descentHoldActive ? 0 : GetXR1().m_setBank);             // in degrees; -90 to +90
        ...
        FireThrusterGroups(targetBank, currentBank, angularVelocity.z, THGROUP_ATT_BANKRIGHT, THGROUP_ATT_BANKLEFT, simdt, 20.0, false, BANK);
        ...
        if (GetXR1().m_initialAHBankCompleted)
        {
            ...
            if ((descentHoldActive == false) && GetXR1().m_holdAOA)
            {
                ...
                if (newPitchTarget != 0)
                {
                    ...
                    requestedColShift = FireThrusterGroups(newPitchTarget, currentPitch, angularVelocity.x, THGROUP_ATT_PITCHUP, THGROUP_ATT_PITCHDOWN, simdt, 20.0, true, PITCH);
                }
                else    // pitch is still OK; let's keep tracking AOA hold
                {
                    // holding AOA
                    requestedColShift = FireThrusterGroups(targetAOA, currentAOA, angularVelocity.x, THGROUP_ATT_PITCHUP, THGROUP_ATT_PITCHDOWN, simdt, 20.0, true, PITCH);
                }
            }
            else  // holding PITCH
            {
                ...
                const double currentPitch = GetVessel().GetPitch() * DEG;   // in degrees
                requestedColShift = FireThrusterGroups(targetPitch, currentPitch, angularVelocity.x, THGROUP_ATT_PITCHUP, THGROUP_ATT_PITCHDOWN, simdt, 20.0, true, PITCH);
            }
        ...
        const double currentYawPositiveThrusterGroupLevel  = GetVessel().GetThrusterGroupLevel(THGROUP_ATT_YAWLEFT); 
        const double currentYawNegativeThrusterGroupLevel = GetVessel().GetThrusterGroupLevel(THGROUP_ATT_YAWRIGHT); 
        ...
        */
        ...
        if ((descentHoldActive == false) && (pilotFiringYawJets == false) && (rudderActive == false))
            KillRotation(angularVelocity.y, THGROUP_ATT_YAWLEFT, THGROUP_ATT_YAWRIGHT, simdt, false, m_lastSetYawThrusterGroupLevels);
    }
...
void AttitudeHoldPreStep::ResetCenterOfLift()
{
    ...
    {
        GetXR1().m_cogShiftCenterModeActive = true;    // a PreStep will handle this
...
double AttitudeHoldPreStep::FireThrusterGroups(const double targetValue, const double currentValue, const double angularVelocity, const THGROUP_TYPE thgPositive, const THGROUP_TYPE thgNegative, const double simdt, double angVelLimit, bool reverseRotation, const AXIS axis, const double masterThrustFrac)
{
    ...
    if (fabs(degreesDelta) > targetDeadZone)
    {
        ...
        if (axis == PITCH)
        {
            sprintf(oapiDebugString(), "thLevel=%lf, degreesDelta=%lf, angVel=%lf, targetAngVel=%lf, cogShiftRequested=%lf", thLevel, degreesDelta, angularVelocity, targetAngVel, retVal);
        }
#endif
        //sprintf(oapiDebugString(), "pitch=%lf, bank=%lf, slip=%lf", GetVessel().GetPitch()*DEG, GetVessel().GetBank() * DEG, GetVessel().GetSlipAngle()*DEG);
    }
    return retVal;
}
...
void AttitudeHoldPreStep::KillRotation(const double angularVelocity, const THGROUP_TYPE thgPositive, const THGROUP_TYPE thgNegative, const double simdt, bool reverseRotation, double *pOutSetThrusterGroupsLevels, const double masterThrustFrac)
{
    ...
    if (pOutSetThrusterGroupsLevels != NULL)
    {
...
void DescentHoldPreStep::clbkPrePostStep(const double simt, const double simdt, const double mjd)
{
    ...
    else if ((customAutopilotMode == AP_DESCENTHOLD) && (m_prevCustomAutopilotMode != AP_NOTSET))
    {
        ...
        if (GetXR1().m_autoLand)    // check whether m_autoLand mode is engaged
        {
            ...
            const double safeMaxShipHoverAcc = maxShipHoverAcc * 0.80;  // reduce effective maxShipHoverAcc to allow a 20% safety margin, and so the final braking isn't so abrupt
        ...
        VECTOR3 v;
        GetVessel().GetHorizonAirspeedVector(v);
        ...
        if (GetXR1().m_autoLand)
        {
            ...
            // BETA-2: TOO AGGRESSIVE: rateDeltaMultiplier *= 5;   // 5 * 2.0 = 10 = minimum multiplier set to reach target acc in 1/10th-second to keep auto-land accurate
...
void AirspeedHoldPreStep::clbkPrePostStep(const double simt, const double simdt, const double mjd)
{
    ...
    if ((GetXR1().m_airspeedHoldEngaged) && (m_prevAirspeedHold != PAH_NOTSET))
    {
        ...
        // get our airspeed speed in meters per second
        ...
        if ((maxMainThrust < zWeight) && (currentAirspeed < targetVelocity))
        {
            ...
            char msg[128];
            sprintf(msg,  "WARNING: insufficient main thrust&available to accelerate further&at this attitude.", targetVelocity, (zWeight / maxMainThrust * 100));
            GetXR1().ShowWarning("Warning Insufficient Main Thrust Available.wav", DeltaGliderXR1::ST_WarningCallout, msg);
        }
...
UMmuPreStep::UMmuPreStep(DeltaGliderXR1 &vessel) : 
    XR1PrePostStep(vessel)
{
}

void UMmuPreStep::clbkPrePostStep(const double simt, const double simdt, const double mjd)
{
    ...
    // NOTE: MMU crew member will only reentry ship if airlockOpen == true, so no need to check it here
    ...
    {
        const char *pName = GetXR1().UMmu.GetLastEnteredCrewName();
...
{
    const double airborneTriggerTime = 0.5;         // assume airborne 1/2-second after wheels up
    double airspeed = GetVessel().GetAirspeed();    // equiv to ground speed when landed
    ...
    if (GetXR1().GetXR1Config()->ParseFailed())
    {
        ...
        {
            sprintf(oapiDebugString(), "Error parsing %s configuration file '%s'; check the '%s' file for details.", VESSELNAME, XR_CONFIG_FILE, XR_LOG_FILE);
        }
        else
        {
            *oapiDebugString() = 0;
    ...
    // NOTE: Orbiter does not set GroundContact() == true until all three touchdown points are down; therefore, we also check altitude here.
    ...
    if ((GetVessel().GroundContact() || GetXR1().GetGearFullyUncompressedAltitude() <= 0.0))
    {
        ...
        // If there is an atmosphere AND APU offline AND airspeed > 5 m/s, show a warning!
        // However, don't check within the first one second of sim time because Orbiter seems to move the vessel slightly on startup.
        if ((airspeed > 5) && (atmPressure > 0) && (simt > 1.0))
            GetXR1().CheckHydraulicPressure(true, false);  // show a warning of APU offline, but do not beep

        // If there is an atmosphere AND AF Ctrl == OFF AND airspeed > 20 m/s, show a warning!
        // However, don't check within the first one second of sim time because Orbiter seems to move the vessel slightly on startup.
        if ((airspeed > 20) && (atmPressure > 0) && (simt > 1.0))
        {
        ...
        {
            VECTOR3 asVector;
            GetVessel().GetHorizonAirspeedVector(asVector);  
            ...
            // Therefore, we check AIRSPEED and TOUCHDOWNVERTICALSPEED here.
            if ((airspeed == 0.0) && (touchdownVerticalSpeed == 0.0))
                previousFrameVerticalSpeed = 0.0;  // the scenario editor moved us

            // NOTE: if touchdownVerticalSpeed < previousFrameVerticalSpeed (meaning, it impact was SOFTER than the previous frame's value), use
            ...
            if (momemtum > LANDING_GEAR_MAX_MOMEMTUM)
            {
                ...
                goto resetForGroundMode;        // just in case more code is added below later
            }
            else  // we have a good landing (or damage was disabled!)
            {
                ...
                const double chirpVolumeFrac = min((0.50 + (0.50 * (airspeed / 100.0))), 1.0);  // minimum volume is 0.5
                GetXR1().PlaySound(DeltaGliderXR1::TouchdownChirp, static_cast<int>((255.0 * chirpVolumeFrac));
                */
                char temp[128];
...
resetForGroundMode:
        ...
        // reset timer in case we are bouncing during takeoff, or if we hovered just enough to bounce
        GetXR1().m_airborneTargetTime = 0;  // reset timer
...
MachCalloutsPreStep::MachCalloutsPreStep(DeltaGliderXR1 &vessel) : 
    XR1PrePostStep(vessel),
    m_previousMach(-1), m_nextMinimumCalloutTime(-1), m_previousSimt(-1)
{
}
...
void MachCalloutsPreStep::clbkPrePostStep(const double simt, const double simdt, const double mjd)
{
    ...
    double mach = GetVessel().GetMachNumber();
    bool groundContact = GetVessel().GroundContact();

    if ((groundContact == false) && (mach <= 0))  // prevent resets when on ground
    {
    ...
    // In addition, disable mach callouts if OAT temperature is not valid (e.g., dynamic pressure too low)
    ...
    if ((simt >= m_nextMinimumCalloutTime) && (m_previousMach >= 0))
    {
        ...
        if ((m_previousMach >= 1.0) && (mach < 1.0))
        {
        ...
        else if ((m_previousMach < 1.0) && (mach >= 1.0))
        {
            if (GetXR1().GetXR1Config()->EnableSonicBoom)
            {
                GetXR1().StopSound(GetXR1().SonicBoom);  // in case it's still playing from before
...
void MachCalloutsPreStep::PlayMach(const double simt, const char *pFilename)
{
    ...
    GetXR1().LoadXR1Sound(GetXR1().MachCallout, pFilename, BOTHVIEW_FADED_FAR);  
...
AltitudeCalloutsPreStep::AltitudeCalloutsPreStep(DeltaGliderXR1 &vessel) : 
    XR1PrePostStep(vessel),
    m_nextMinimumCalloutTime(-1), m_previousSimt(-1)
{
}
...
void AltitudeCalloutsPreStep::clbkPrePostStep(const double simt, const double simdt, const double mjd)
{
    ...
    if ((simt >= m_nextMinimumCalloutTime) && (GetXR1().m_preStepPreviousGearFullyUncompressedAltitude >= 0))
    {
        ...
        {
            // do not play the callout if vertical speed is too high; i.e., we are going to crash!
...
void AltitudeCalloutsPreStep::PlayAltitude(const double simt, const char *pFilename)
{
    ...
    GetXR1().LoadXR1Sound(GetXR1().AltitudeCallout, pFilename, RADIO_SOUND);  // audible outside vessel as well
...
void DockingCalloutsPreStep::clbkPrePostStep(const double simt, const double simdt, const double mjd)
{
    ...
    // enable/disable the default OrbiterSound docking callouts
    const bool dockingCalloutsEnabled = (GetXR1().nose_status == DOOR_OPEN);
    GetXR1().XRSoundOnOff(PLAYDOCKINGSOUND, dockingCalloutsEnabled);
    ...
    {
        // if within 100 meters, warn pilot if nosecone is closed; do NOT warn him if nosecone is OPEN or OPENING
        if ((distance < m_previousDistance) && (distance < 100) && ((GetXR1().nose_status != DOOR_OPEN) && (GetXR1().nose_status != DOOR_OPENING)))
        {
            char msg[128];
            sprintf(msg, "ALERT: %s is closed!", NOSECONE_LABEL);
            GetXR1().ShowWarning("Warning Nosecone is Closed.wav", DeltaGliderXR1::ST_WarningCallout, msg);
        }
...
void UpdatePreviousFieldsPreStep::clbkPrePostStep(const double simt, const double simdt, const double mjd) 
{
    ...
    GetXR1().m_preStepPreviousAirspeed = GetXR1().GetAirspeed();
...
void ScramjetSoundPreStep::clbkPrePostStep(const double simt, const double simdt, const double mjd)
{
    ...
    // flow should be from 170 (idle) to 255
    int volume = 170 + static_cast<int>(flowLevel * 75);
...
// This should be invoked FIST in the PreStep order to ensure that the internal tanks stay full across the timestep.
...
void DrainBayFuelTanksPreStep::clbkPrePostStep(const double simt, const double simdt, const double mjd) 
{
    ...
    if (GetXR1().m_pPayloadBay == NULL)
        return; 

    FlowBayFuel(GetXR1().ph_main, false);
    FlowBayFuel(GetXR1().ph_scram, GetXR1().m_SCRAMTankHidden);
...
void DrainBayFuelTanksPreStep::FlowBayFuel(const PROPELLANT_HANDLE ph, const bool isTankHidden)
{
    ...
    if (isTankHidden && (GetXR1().m_pPayloadBay->GetPropellantMass(pt) <= 0))  // < 0 for sanity check
    {
