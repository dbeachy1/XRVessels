...
//
// XR5PreSteps.cpp
// Class defining custom clbkPreStep callbacks for the XR5 Vanguard
// ==============================================================
...
SetCenterOfLiftPreStep::SetCenterOfLiftPreStep(XR5Vanguard &vessel) : 
...
void SetCenterOfLiftPreStep::clbkPrePostStep(const double simt, const double simdt, const double mjd)
{
    ...
    if (GetXR5().m_customAutopilotMode == AP_ATTITUDEHOLD)
        return;     // don't change anything

    // don't do anything unless AUTO COG mode or RECENTER mode is engaged
    if ((GetXR5().m_cogShiftAutoModeActive == false) || GetXR5().m_cogShiftCenterModeActive)
        return;     // don't change anything
    ...
    if (GetXR5().CheckHydraulicPressure(false, false) == false)     // no sounds here -- we get called at each timestep!
        return;
    ...
    // get our velocity
    const double airspeed = GetXR5().GetAirspeed();     // in meters-per-second
    ...
    // OLD: GetXR5().m_centerOfLift = HIGHSPEED_CENTER_OF_LIFT + (deployFrac * movementRange);  // lift is moved FORWARD (+Z axis) as flaps are DEPLOYED
    // OLD: GetVessel().EditAirfoil(GetXR5().hwing, 0x01, _V(0, 0, GetXR5().m_centerOfLift), nullptr, 0, 0, 0);

    sprintf(oapiDebugString(), "airspeed=%lf, deployFrac=%lf, centerOfLift=%lf", airspeed, deployFrac, GetXR5().m_centerOfLift);
}
#endif
...
XR5NosewheelSteeringPreStep::XR5NosewheelSteeringPreStep(XR5Vanguard &vessel) : 
    XR5PrePostStep(vessel),
    m_steeringActiveDuringPrevTimestep(false)
{
}

void XR5NosewheelSteeringPreStep::clbkPrePostStep(const double simt, const double simdt, const double mjd)
{
    if (GetXR5().IsCrashed()) 
    {
    ...
    if ((GetXR5().gear_status != DoorStatus::DOOR_OPEN) || (GetXR5().m_noseGearProc == 1.0))
    {
        ...
        {
            GetXR5().SetXRAnimation(GetXR5().m_animNosewheelSteering, 0.5);  // center
    ...
    if (GetXR5().CheckHydraulicPressure(false, false) == false)      // no sound or message here: this is invoked each timestep
    {
        GetVessel().SetNosewheelSteering(false);
        GetXR5().SetXRAnimation(GetXR5().m_animNosewheelSteering, 0.5);    // recenter since steering is inactive
        return;
    }
    ...
    GetXR5().SetXRAnimation(GetXR5().m_animNosewheelSteering, animState);
    
    m_steeringActiveDuringPrevTimestep = true;

    GetXR5().AmplifyNosewheelSteering();  // rotate the ship to fix poor nosewheel steering performance inherent in all Orbiter vessels by default
}
